...
class _CaregiverPatientList extends State<CaregiverPatientList> {
  ...
  /// Starts a conversation with the specified patient.
  ///
  /// Navigates to the ChatRoomScreen with the patient's user ID and name.
  /// Only works if the patient has a valid user ID.
  ///
  /// Parameters:
  /// * [patient] - The patient to start a conversation with
  void _startConversationWithPatient(Patient patient) {
    if (patient.userId == null) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Cannot start conversation: Patient user ID not available',
            ),
            backgroundColor: Colors.orange,
            behavior: SnackBarBehavior.floating,
          ),
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return PatientCard(
                          ...
                          onMessageTap: () {
                            _startConversationWithPatient(patient);
                          },
                        );
                      },
                    ),
