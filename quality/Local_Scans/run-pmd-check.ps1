...
$pmdRawFile = Join-Path -Path $reportsDir -ChildPath "pmd.xml"
$cpdRawFile = Join-Path -Path $reportsDir -ChildPath "pmd-cpd.xml"
...
if (Test-Path $pmdRawFile) {
    Remove-Item -Path $pmdRawFile -Force
}
if (Test-Path $cpdRawFile) {
    Remove-Item -Path $cpdRawFile -Force
}
...
$backendCorePath = Join-Path -Path $rootPath -ChildPath "backend\core"
$possiblePmdFiles = @(
    (Join-Path -Path $backendCorePath -ChildPath "target\pmd.xml"),
    (Join-Path -Path $backendCorePath -ChildPath "target\pmd-result.xml"),
...
foreach ($file in $possiblePmdFiles) {
    if (Test-Path $file) {
        ...
        # Copy raw file to reports directory
        Copy-Item -Path $file -Destination $pmdRawFile -Force
        Write-Host "Copied raw file to: $pmdRawFile" -ForegroundColor Green
...
$backendCorePath = Join-Path -Path $rootPath -ChildPath "backend\core"
$possibleCpdFiles = @(
    (Join-Path -Path $backendCorePath -ChildPath "target\pmd-cpd.xml"),
    (Join-Path -Path $backendCorePath -ChildPath "target\cpd.xml"),
    (Join-Path -Path $backendCorePath -ChildPath "target\pmd-cpd-result.xml"),
...
foreach ($file in $possibleCpdFiles) {
    if (Test-Path $file) {
        ...
        # Copy raw file to reports directory
        Copy-Item -Path $file -Destination $cpdRawFile -Force
        Write-Host "Copied raw file to: $cpdRawFile" -ForegroundColor Green
