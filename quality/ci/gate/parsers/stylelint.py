# File: quality/ci/gate/parsers/stylelint.py
# ==========================================================
# stylelint.py
# ----------------------------------------------------------
...
#   warning → Advisory violation.
#
# Severity Mapping (Stylelint → Normalized):
#   error   → high
#   warning → medium
#   <unknown> → low
#
# Stylelint JSON Structure (array of file result objects):
#   [
#     {
...
#
# Note:
#   Stylelint calls all findings "warnings" in its JSON structure
#   regardless of severity. The actual severity is in the
#   "severity" field of each warning object.
# ==========================================================
...
def parse_stylelint(raw_dir: Path) -> dict:
    ...
    try:
        ...
        # Stylelint JSON is an array of per-file result objects
        file_results = data if isinstance(data, list) else []
        findings     = []

        for file_result in file_results:
            file_path = file_result.get("source", "unknown")
            # Stylelint uses "warnings" for all findings regardless of severity
